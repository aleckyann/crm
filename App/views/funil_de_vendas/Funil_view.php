<div class="main-container">
    ...
    <div class="breadcrumb-bar navbar bg-white sticky-top">
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item"><a href="<?=base_url('funil-de-vendas') ?>">Funil de vendas</a></li>
    ...
    <div class="container-kanban">
        <div class="container page-header d-flex justify-content-between align-items-start m-0">
            <div class="col-lg-11">
                <h1>Funil de vendas</h1>
                <span class="lead">Nossas prospecções e acompanhamentos de leads</span>
...
function initFunil() {
    ...
    mrKanban.on('sortable:stop', function(e) {
        $.ajax({
            method: "POST",
            url: "<?=base_url('funil-de-vendas/change')?>",
            data: {
...
function createLead() {
    ...
    $.ajax({
        method: "POST",
        url: "<?=base_url('funil-de-vendas/create_card')?>",
        data: {
            <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
...
function openLead(funil_id) {
    ...
    $.ajax({
        method: "POST",
        url: "<?=base_url('funil-de-vendas/read')?>",
        data: {
            <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
            funil_id: funil_id
        }
        ...
        $.each(resultado, function(i, result){
            ...
            <div class="card card-note">
                <div class="card-header">
                    <div class="media align-items-center">
                        <div class="media-body">
                            <h6 class="mb-0" data-filter-by="text">${result.fvn_tipo}</h6>
                        </div>
                    </div>
...
function addNota() {
    ...
    const titulo = Swal.fire({
      ...
      preConfirm: () => {
          ...
          $.ajax({
              method: "POST",
              url: "<?=base_url('funil-de-vendas/create_nota')?>",
              data: {
                  <?=$this->security->get_csrf_token_name()?>: "<?=$this->security->get_csrf_hash()?>",
                  funil_id: funil_id,
                  fvn_tipo: nota_tipo,
                  fvn_descricao: nota_descricao
              }
          })
          .done(function(resultado) {
              let nova_nota = `
